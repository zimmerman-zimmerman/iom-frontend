...
const Slider = (props) => {
  ...
  const sidebar = (
    <List>{menuItems.map((item, index) => {
      const className = item.url === location.pathname || item.url === location.pathname.substr(0, slashIndex) ? "active" : "";
      return (<List.Item key={index} className={className}><Link to={item.url}><h3>{item.text}</h3></Link></List.Item>)})}
    </List>
  );
  return (
    <Fragment>
