...
class BaseFilter extends Component {
  ...
  filter(data, valueKey = "value") {
    ...
    if (this.state.dataRange.length > 0 && dataSource) {
      return dataSource.filter(
        (d) =>
          d[valueKey] >= this.state.dataRange[0] &&
          d[valueKey] <= this.state.dataRange[1]
      );
    }
    return dataSource;
  }
  ...
  actionRequest(params, groupBy, request) {
    const dispatch = this.props.dispatch
      ? this.props.dispatch
      : this.props.rootComponent.props.dispatch;
        ...
        if (filters.values["sector type"]) {
          ...
          if (values["sector"]) {
            values["sector"] = values["sector"]
              .concat(",")
              .concat(filters.values["sector type"]);
          } else {
