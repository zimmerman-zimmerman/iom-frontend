...
class ProjectLocation extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (!this.state.cebCategory && data && cebCategories) {
      this.getCEBCategory(
        get(data, "sectors"),
        get(cebCategories, "data.content")
      );
    }
  ...
  render() {
    ...
    const aidType = get(
      this.props.projectTransactions,
      "data.results[0].aid_type.name",
      ""
    );
    const financeType = get(
      this.props.projectTransactions,
      "data.results[0].finance_type.name",
      ""
    );
    const flowType = get(
      this.props.projectTransactions,
      "data.results[0].flow_type.name",
      ""
    );
    const websiteLink = get(data, "contact_info[0].website", "-").includes(
      "http"
    )
    ...
    const fundingDonor = find(
      get(data, "participating_organisations", []),
      (org) => org.role.code === "1"
    );

    let donorExtra = get(
      donorGroupJson,
      get(data, get(fundingDonor, "ref", null))
    );
    ...
    const fields = [
      {
        ...
        rows: [
          {
            value: (
              <Col xs={12} className="flag-country">
                {get(data, "recipient_countries", []).length > 0 ? (
                  <React.Fragment>
                    <div className="flag-icon">
                      {countryFlag !== "" && (
                        <ReactCountryFlag code={countryFlag} svg />
                      )}
                    </div>
                  ...
                  <span className="region-name">
                    {get(data, "recipient_regions[0].region.name", "")}
                  </span>
                )}
              </Col>
            ),
          ...
          {
            name: {
              id: "project.location.fields.iati.reference",
              defaultMessage: "IATI reference:",
            },
            value: <span>{get(data, "iati_identifier", "-")}</span>,
          },
          {
            name: {
              id: "project.location.fields.funding",
              defaultMessage: "Funding donor:",
            },
            value: (
              <Link
                to={`/donors/${donorExtra}${get(
                  fundingDonor,
                  "primary_name",
                  ""
                )}`}
              >
                {get(fundingDonor, "primary_name", "-")}
              </Link>
            ),
          },
          {
            name: {
              id: "project.location.fields.donor.type",
              defaultMessage: "Donor type:",
            },
            value: <span>{get(fundingDonor, "type.name", "-")}</span>,
          },
          {
            name: {
          ...
          {
            name: {
              id: "project.location.fields.status",
          ...
          {
            name: {
              id: "project.location.fields.type",
              defaultMessage: "Project type:",
            },
            value: (
              <div className={classes.sectorLinks}>
          ...
          {
            name: {
              id: "project.location.fields.ceb.category",
              defaultMessage: "CEB Category:",
            },
            value: this.state.cebCategory ? this.state.cebCategory : "-",
          },
          {
            name: {
              id: "project.location.fields.dac",
          ...
          {
            name: {
              id: "project.location.fields.service",
          ...
          {
            name: {
              id: "project.location.fields.finance.type",
              defaultMessage: "Finance type:",
            },
            value: <span>{financeType}</span>,
          },
          {
            name: {
              id: "project.location.fields.flow.type",
          ...
          {
            name: {
              id: "project.location.fields.aid.type",
              defaultMessage: "Aid type:",
            },
      ...
      {
        title: {
          id: "project.location.contact.title",
          defaultMessage: "Contact info",
        },
        rows: [
          {
            name: {
              id: "project.location.contact.phone",
          ...
          {
            name: {
              id: "project.location.contact.email",
          ...
          {
            name: {
              id: "project.location.contact.address",
            ...
            value: (
              <span>
                {get(
                  data,
                  "contact_info[0].mailing_address.narratives[0].text",
                  "-"
                )}
              </span>
            ),
          ...
          {
            name: {
              id: "project.location.contact.website",
          ...
          <Col xs={12} md={6} className="right">
            {countryData && countryTransactionData ? (
              <GeoMap
                data={formatMapData(
                  countryData,
                  countryTransactionData.activity_count,
                  countryTransactionData.value
                )}
                zoom={6}
                country="nl"
                height={450}
...
export default injectSheet(styles)(
  withRouter(connect(mapStateToProps)(ProjectLocation))
);
