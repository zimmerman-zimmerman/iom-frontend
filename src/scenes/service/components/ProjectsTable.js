  ...
  render() {
    ...
    const columns = [
      {
        title: (
          <SortHeader
            title={intl.formatMessage({
              id: "service.projects.header.project",
        ...
        render: (obj) => (
          <Link to={`/projects/${obj.id}`}>
            {getProjectTitle(obj.title.narratives)}
          </Link>
        ),
      },
      {
        title: (
          <SortHeader
            title={intl.formatMessage({
              id: "service.projects.header.donor",
        ...
        dataIndex: "participating_organisations",
        render: (poObj) => {
          console.log(poObj);
          const obj = find(poObj, (po) => get(po, "role.code", "0") === "1");
          if (obj) {
      ...
      },
      {
        title: (
          <SortHeader
            title={intl.formatMessage({
              id: "service.projects.header.value",
      ...
      },
      {
        title: (
          <SortHeader
            title={intl.formatMessage({
              id: "country.table.projects.header.status",
      ...
      },
      {
        ...
        key: "start_date",
        render: (activityDates) => (
          <div className={classes.dates}>{getDate(activityDates, "start")}</div>
        ),
      },
      {
        title: (
        ...
        key: "end_date",
        render: (activityDates) => (
          <div className={classes.dates}>{getDate(activityDates, "end")}</div>
        ),
      },
    ];
    ...
    return (
      <div className={classes.projectsTable}>
        ...
        <Table
          dataSource={data ? this.addKey(data.results) : null}
          columns={columns}
          pagination={false}
          scroll={{ x: 650 }}
        ...
        {total > 5 && (
          <Pagination
            ...
            forcePage={
              get(this.props.rootComponent.state.filters.values, "page", 1) - 1
            }
          />
        )}
      </div>
...
export default injectSheet(styles)(
  connect(mapStateToProps)(injectIntl(ProjectsTable))
);
