...
class TableDonors extends React.Component {
  ...
  render() {
    ...
    const columns = [
        ...
        title: (
          <SortHeader
            title={intl.formatMessage({
              id: "country.table.donors.header.donors",
      ...
      },
      {
        title: (
          <SortHeader
            title={intl.formatMessage({
              id: "country.table.donors.header.total",
    ...
    const data = paginate(
      this.state.page,
      this.state.pageSize,
      filter(this.props.data, { participating_organisation_role: "1" })
    );
    return (
      <Fragment>
        <Table
        ...
        {this.props.data && this.props.data.length > this.state.pageSize && (
          <Pagination
            pageCount={Math.ceil(this.props.data.length / this.state.pageSize)}
            onPageChange={(value) =>
